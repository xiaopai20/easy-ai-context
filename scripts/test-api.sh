...
echo "5. POST /mcp - tools/call context_list_paths"
curl -s -w "\n[HTTP %{http_code}]" "${AUTH[@]}" -X POST "$API/mcp" \
  -H "Content-Type: application/json" \
  -d '{"jsonrpc":"2.0","method":"tools/call","params":{"name":"context_list_paths","arguments":{}},"id":1}'
echo -e "\n"

echo "6. POST /mcp - tools/call context_set"
curl -s -w "\n[HTTP %{http_code}]" "${AUTH[@]}" -X POST "$API/mcp" \
  -H "Content-Type: application/json" \
  -d '{"jsonrpc":"2.0","method":"tools/call","params":{"name":"context_set","arguments":{"path":"projects/easy-ai","content":"hello"}},"id":2}'
echo -e "\n"

echo "7. POST /mcp - tools/call context_get"
curl -s -w "\n[HTTP %{http_code}]" "${AUTH[@]}" -X POST "$API/mcp" \
  -H "Content-Type: application/json" \
  -d '{"jsonrpc":"2.0","method":"tools/call","params":{"name":"context_get","arguments":{"path":"projects/easy-ai"}},"id":3}'
echo -e "\n"
